...
// Feature: Agentic memorial concierge that drafts obituary, plans service, and notifies attendees
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticMemorialConciergeThatDraftsObituaryPlPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-memorial-concierge-that-drafts-obituary-pl/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic memorial concierge that drafts obituary, plans service, and notifies attendees</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-memorial-concierge-that-drafts-obituary-pl/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}

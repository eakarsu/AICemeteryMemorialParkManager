...
export default function NoFuneralHomeCoronerSystemIntegrationPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-funeral-home-coroner-system-integration/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No funeral-home / coroner system integration</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-funeral-home-coroner-system-integration/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}

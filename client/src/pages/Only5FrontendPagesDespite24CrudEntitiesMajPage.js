...
export default function Only5FrontendPagesDespite24CrudEntitiesMajPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-only-5-frontend-pages-despite-24-crud-entities-maj/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Only 5 frontend pages despite 24+ CRUD entities — major UI gap</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-only-5-frontend-pages-despite-24-crud-entities-maj/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}

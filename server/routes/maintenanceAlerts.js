/*
 * Maintenance alerts: Real-time alerts with photo evidence
 *
 * Custom Feature Suggestion (apply pass 6 — audit batch_01)
 * Suggestion: Real-time alerts with photo evidence
 *
...
router.post('/run', async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({ error: 'AI unavailable', missing: 'OPENROUTER_API_KEY', feature: 'maintenance_alerts' });
    }
    ...
    const systemPrompt = `You are a $'cemetery maintenance alert generator'. Respond with strict JSON.`;
    const aiResp = await callOpenRouter(userPrompt, systemPrompt);
    const content = aiResp?.choices?.[0]?.message?.content ?? aiResp?.raw ?? '';
    const parsed = parseAIJson(content);
    return res.json({ feature: 'maintenance_alerts', model: aiResp?.model, parsed, raw: content });
  } catch (err) {
...
router.get('/health', (req, res) => res.json({ feature: 'maintenance_alerts', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
